...
资产计费效果的 `config` 可包含 `applicableSessionLabels`、`applicablePricingConfigIds`、`applicableRuleIds`。结算时只有当前计时名称、费用项所属计费方案和费用项所属规则匹配时，效果才会作用到那部分费用。后台 UI 会用中文控件生成这些配置，员工不需要手写 JSON。
