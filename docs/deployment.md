...
* **构建新版后台 (`prism-dashboard`)**（推荐）：
  ...
  `web/_headers` 会让 Cloudflare Pages 缓存 Wasm 运行文件 4 小时，避免每次进入
  都重新下载运行时；根构建脚本会将它复制到 `build/web/_headers`，HTML 仍保持
  重新验证，bootstrap 也不会跨发布使用旧构建配置。生产 Pages 构建使用默认
  JavaScript 编译，不加 `--wasm`。
...
2. 首次进入或在登录界面，您需要配置 **API Base URL**（如本地的 `http://localhost:8787` 或云端的 Worker 域名）。
   生产环境不会猜测或自动连接 API，服务器地址默认为空；只有点击「连接服务器」后才会检查后端并显示登录或初始化表单。单个请求最多等待 10 秒。本地 Web 开发默认填写 `http://localhost:8787`。
