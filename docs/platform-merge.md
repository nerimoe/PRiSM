...
Stop old writers and take a backup before production migration. Do not apply migration 0017 to an ArcadeLink database: the destination is a migrated PRiSM database. Import ArcadeLink rows separately with explicit columns, preserving IDs. The merger starts every imported shop with optional location checks off; the owner enables the unified policy after reviewing the mapping. Store mapping and staff-account mapping must be explicit. Do not merge by nickname or QQ across shops.
...
- 443 Bun tests, TypeScript checks, 47 Koishi tests, Web build and Wrangler dry-run passed. Transactional checkout, shop isolation, QQ proof, current staff membership, optional location, idempotent money operations and concurrent ticket claiming have runnable regression checks.
...
Paths are relative to the manifest. `staffAccounts` can be empty; global shop owners are then provisioned on first access. Run `python3 scripts/merge-platform.py /private/manifest.json /private/new-platform.sqlite`. The output must not exist. The command opens source snapshots read-only, verifies schemas and every source table, checks foreign keys, writes migration tracking for 0016/0017 and creates the destination with private permissions. Imported shops keep billing disabled and optional location checks off until an owner reviews the Bot, entry pricing and location policies. The JSON report includes `legacyDeviceRegistries`; resolve each entry before cutover by running `scripts/import-legacy-devices.ts`. That importer encrypts the old HA URL/token, creates deterministic logical devices, maps old command/state/connection references, and can emit a transaction SQL file with `--sql-out` for D1. Keep `URL_ENCRYPTION_KEY` out of manifests, logs and Git. Existing currency codes, identities, operation history and balances are preserved.

The standalone SQLite server automatically backs up and transactionally upgrades a pre-0016 local database on startup. Embedded runtime users must explicitly migrate an old database before schema initialization. For a production cutover, stop all old writers, take fresh snapshots, review actual store/staff mappings, rehearse again, resolve legacy device reports and restore the verified result into a new D1 database. Preserve the original databases for rollback; do not overwrite either source. Configure the existing encrypted-URL key, session secret, MuNET credentials, passkey origin/RP ID and native association IDs before switching the `link.neri.moe` route. New hosted billing APIs coexist with the standalone legacy adapters; legacy billing clients continue to use their old single-store deployment until migrated. See [迁移复盘](migration-incident-2026-09-13.md) for the failure modes and the exact replay order.
...
**Device-free entry.** Migration **0023_remote_entry.sql** adds `remote_entry_enabled` to `shop_billing_settings`, defaulting to off. When a shop opts in, `POST /api/v1/shops/:shopCode/player/remote-entry` starts an ordinary entry session with no machine ticket. It still requires explicit consent and, when location policy is on, an inside fix — the same `checkin` rule as a scanned entry. The command reuses the machine entry path (entry pricing rules, `player.entry` operation lease, `entry` label) and is idempotent through the shared player-operation lease, so a second attempt while a session runs returns the running one. Shops that do not opt in keep returning `DEVICE_QR_REQUIRED`. Merchant settings expose the opt-in as 「允许无设备自助入场」; a settings update that omits the field preserves the stored value.
...
在店页参考原 Flutter 管理面板，使用玩家列表与账单双栏布局，小屏上下排列。列表显示时长、应付与状态；账单按会话展示金额，展开查看实际计费时段、费用及封顶。支持按状态或进行中的方案组合分组，不重复计数。文案使用“账单”“应付”“余额”。复用 live-players 明细，不增加逐人请求；停止与结账复用既有接口。完整计算顺序见 [当前计费说明](billing-flow.md)。
