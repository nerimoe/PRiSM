...
- `packages/rpc`：共享的 HTTP RPC 接口契约与轻量级 Fetch 请求器，供机器人和未来的 Web 客户端使用，防止各客户端自行拼接 URL 与认证 Header。游戏机软件使用独立的 Machine WebSocket 协议。
- `packages/prism-dashboard`：新的 Flutter Web 后台，Dart 包名为 `prism_dashboard`。它以玩家现场运营为中心，消费共享 RPC 与员工读模型，不在旧 `admin-flutter` 上继续叠加 UI。现场工作台按已确认的 v9 shell 实现：230px 分组文本侧栏、玩家一级表格、选中玩家后的 session 明细和玩家级账单。侧边栏保持一项入口对应一个后台模块，模块内部细分使用页面标签页承载，避免多个入口映射同一页面。后台列表详情页采用固定宽度列表加弹性详情区，避免把大量信息压进细长右栏。玩家档案把资产流水、计时记录和兑换记录都作为可审计的二级记录展示；兑换记录通过员工读模型返回礼物名称、兑换码和兑换时间。现场账单保留 `pricingCharges` 紧凑摘要，并在展开后按 `pricingSegments` 展示实际计费 ISO 时间范围和规则时段；方案内封顶只在已达到时显示。全局封顶按锚定的 `globalCapWindows` 分组，窗口详情展示完整范围、历史已计入金额及参与项目；达到封顶时展示封顶后的最终金额，未达到时展示当前计入金额，绝不把封顶调整差额渲染成应收金额。计费配置按真实业务拆分为按时计费方案、全局封顶时间轴和固定收费方案：按时计费编辑多条优先级时段、星期/指定日期、单价、宽限和方案内封顶，并调用后端时间轴预览；全局封顶选择参与的按时计费方案，只编辑封顶时段和封顶金额；计费方案放在编辑器顶部，桌面布局下圆环预览和当天时段列表固定在左侧，方案信息与时段编辑放在右侧，预览刷新时保留上一版圆环，避免影响正在输入的表单；已归档方案和时段不混在可用列表中，而是进入独立归档区域，可从该区域恢复；固定收费只保存一次性名称和金额，不伪装成时间规则。资产与礼物模块拆成 `资产定义`、`计费效果`、`礼物`、`兑换码` 四个业务页签，每个页签只展示自己的主操作，避免跨上下文按钮把员工带到错误表单。资产定义使用列表加详情编辑面板，员工可以点开资产查看完整信息并再次编辑，不再依赖大弹窗承载复杂配置。计费效果有独立列表和归档区域，创建表单按基础信息、结算方式、适用范围和生效时间分区。礼物页面同样使用列表/详情结构，创建礼物时打开同页草稿面板，详情里用资产名称展示礼物内容，并展示绑定兑换码的可用、已用完、已撤销数量。已绑定兑换码或已有兑换历史的礼物按版本化处理：员工应归档旧礼物并新建替代礼物，避免旧 CDK 的含义被直接修改。兑换码页面只展示汇总和最近少量预览，支持按绑定礼物、使用状态、玩家、兑换码、兑换时间和可用时间筛选，并按创建时间、兑换时间、过期时间、使用次数或礼物名称排序；数量 chip 本身作为使用状况筛选入口；每条兑换码都提供独立复制到剪贴板的操作；已用兑换码会显示使用者和使用时间，并提供跳转玩家档案的入口。生成兑换码和配置计费效果都使用同页工作区面板，不再依赖大弹窗承载复杂配置。礼物和服务项目同样把已归档对象折叠到独立区域，主列表只展示当前可用对象。完整后台模块覆盖现场运营、玩家档案、资产与礼物、计费配置、服务项目与订单、设备看板、营业报表、员工与系统，并以真实员工 RPC 读写为准，不使用前端假数据填充业务流程。
...
- `packages/bot-client`：可复用的机器人端 RPC 客户端，涵盖身份、场次、计费、钱包、设备动作、扫码和兑换。
- `packages/koishi-plugin`（git 子模块，独立仓库 `koishi-plugin-prism`）：基于 `@prism/bot-client` 之上封装的 Koishi 机器人命令连线。
...
- 设备动作授权、投币冷却、Machine WebSocket 投递与响应 ACK 状态流转。动作边界使用 `facility/home_assistant` 与 `game_machine/machine_ws` 两套语义：电源、空调、门禁等设施动作进入设施执行器，投币和 Aime 扫描等游戏机器动作进入机器软件执行器。玩家触发 `coin`、`aime.scan`、`power.on` 或 `power.off` 必须处于活跃计费 session；受信任集成可以把管理员开关机请求明确标记为员工动作。机器连接会保存在线状态、能力列表、最后心跳时间和断开时间；执行失败的 ACK 会把错误信息写入命令 payload 供员工后台审计。
...
- 基于 Hono + `@prism/rpc` 的员工 API，以及新的 Flutter Web 后台 `prism_dashboard`。`/admin` 只保留部署提示页，正式管理端从 `packages/prism-dashboard` 构建。
...
- 除 Home Assistant/TTLock 网关/AimeIO 扫码/本地投币机之外的更多硬件厂商预设集成。
- 编译期的 Hono 路由与响应体强类型自动生成（目前以 `@prism/rpc` 为核心手工维护映射）。
- 更复杂的报表图表与批量操作。
- 计费配置的多版本灰度回滚管理。
